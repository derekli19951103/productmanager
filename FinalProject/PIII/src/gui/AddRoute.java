...
public class AddRoute extends JFrame{
	...
	public AddRoute(Project p,int sessionid) {
		super("Add a Route");
        this.project=p;
		...
		addLabel("City 1", panel);
		addBox(textFields[0], panel);
		addLabel("City 2", panel);
		addBox(textFields[1], panel);
		addLabel("Distance Between", panel);
	...
	private class ButtonHandler implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource()==save){
				project.addRoute(textFields[0].getText(), textFields[1].getText(), Integer.parseInt(textFields[2].getText()), sessionid);
			}else{

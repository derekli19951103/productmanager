...
public class ProjectV1 extends Project {
	...
	public boolean addToShoppingCart(int productID, int quantity, int sessionID, int custID) {
		...
		if (active.containsKey(sessionID)) {
			if (active.get(sessionID) instanceof api.Shopper) {
			    ...
			    boolean flag= shop.addProductToCart(productID, quantity);
			    return flag;
	...
	public List<ShoppingCartItem> getShoppingCart(String userName) {
		...
		Shopper shopper =(Shopper) users.get(userName);
		ShoppingCart cart = shopper.cart;
		Set<Integer> set = cart.products.keySet();
		for(int i:set){
		}

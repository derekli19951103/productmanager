...
public class DistributionCentre implements Serializable{
	...
	public void subQty(int prodID, int num){
		...
		int newQty = oldQty-num;
		if(newQty <0){
			products.replace(prodID, 0);
		}
		else{products.replace(prodID, oldQty,newQty);}
	}
	/**
	 * Add product to the distribution.
	 * @param prodID the ID of the product
	 * @param num the quantity of the product
	 */
	
	public void addProduct(int prodID,int num){
		this.products.put(prodID, num);
	}
